...
from calibrationtools.output_contracts import CSVColumnOutputContract


def _runner(mrp_run_func):
    return MRPOutputRunner(
        "/tmp/example_model.mrp.toml",
        output_contract=CSVColumnOutputContract(
            filename="output.csv",
...
def test_mrp_output_runner_parses_csv_column():
...
def test_mrp_output_runner_raises_on_failed_run():
    def fake_mrp_run(config_path, overrides):
        return RunResult(
            exit_code=1,
            stdout=b"",
...
def test_mrp_output_runner_requires_csv_column():
    def fake_mrp_run(config_path, overrides):
        return RunResult(
            exit_code=0,
...
def test_mrp_output_runner_converts_numpy_scalars():
    def fake_mrp_run(config_path, overrides):
...
def test_mrp_output_runner_ignores_stdout_preamble():
    def fake_mrp_run(config_path, overrides):
        return RunResult(
            exit_code=0,
            stdout=(
...
def test_mrp_output_runner_uses_staged_input_and_output_dirs(tmp_path):
...
def test_mrp_output_runner_preserves_filesystem_output_for_direct_params(
    tmp_path,
):
    run_output_dir = tmp_path / "output"
...
def test_mrp_output_runner_absolutizes_staged_input(tmp_path, monkeypatch):
