...
def test_sampler_default_max_concurrent_simulations_is_configured(
    ...
    assert recorded["max_workers"] == 10


def test_sampler_max_concurrent_simulations_sets_default_worker_count(
    K, P, Vnorm, monkeypatch
):
    recorded = {}
    real_executor = sampler_module.ThreadPoolExecutor
...
def test_sampler_constructor_preserves_legacy_positional_order(K, P, Vnorm):
    sampler = ABCSampler(
        5,
        [0.5],
        P,
    ...
    assert sampler.outputs_to_distance is outputs_to_distance
    assert sampler.target_data == 0.75
    assert isinstance(sampler.model_runner, DummyModelRunner)
    assert sampler.perturbation_kernel is K
    assert sampler.particles_to_params is particles_to_params
    assert sampler.max_attempts_per_proposal == 11
    assert sampler.max_proposals_per_batch == 12
    assert sampler.max_concurrent_simulations == 3
    assert sampler.entropy == 123
    assert sampler.verbose is False
    assert sampler.keep_previous_population_data is False
    assert sampler.results_inherit_entropy_only is True
    assert sampler._priors is P
