...
from calibrationtools.cloud.task_payload import ResolvedSharedAsset
...
class FakeCloudClient:
    ...
    def upload_files(self, **kwargs: Any) -> None:
        self.calls.append(("upload_files", kwargs))
...
def test_cloud_runner_validates_shared_assets_before_client_creation(
    tmp_path: Path,
):
    calls: list[tuple[str, Any]] = []
        ...
        make_cloud_runner(
            ...
            shared_assets=(
                ResolvedSharedAsset(
                    ...
                    source_path=missing,
                    is_dir=False,
                    remote_blob_dir="assets/reference",
                    remote_mount_path="",
                ),
            ),
        )

    assert not any(name == "create_client" for name, _ in calls)


def test_initialize_cloud_session_uploads_shared_file_once(tmp_path: Path):
    calls: list[tuple[str, Any]] = []
    ...
    uploads = [details for name, details in calls if name == "upload_files"]
    assert len(uploads) == 1
    assert uploads[0]["files"] == "reference.json"
    assert uploads[0]["location_in_blob"] == (
        "assets/reference/session-abc123"
    )
