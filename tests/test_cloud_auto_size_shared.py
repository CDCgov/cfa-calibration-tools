...
def test_peak_rss_process_tree_includes_child_usage(monkeypatch):
    def fake_getrusage(scope):
        if scope == 0:
            return SimpleNamespace(ru_maxrss=10)
        return SimpleNamespace(ru_maxrss=20)

    monkeypatch.setattr(
        "calibrationtools.cloud.auto_size.resource.getrusage",
        fake_getrusage,
    )
    monkeypatch.setattr(
        "calibrationtools.cloud.auto_size.sys.platform",
        "linux",
    )

    assert _peak_rss_bytes(CloudAutoSizeMemoryScope.PROCESS_TREE) == 20 * 1024
