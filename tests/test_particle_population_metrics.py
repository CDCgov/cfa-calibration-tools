...
def test_get_quantiles(metrics):
    quantiles = metrics.get_quantiles(
        quantiles=[0.25, 0.5, 0.75], params=["param1", "param2"]
    )
...
def test_get_credible_intervals(metrics):
    intervals = metrics.get_credible_intervals(
...
def test_get_covariance_matrix(metrics):
    covariance_matrix = metrics.get_covariance_matrix(
...
def test_get_correlation_matrix(metrics):
    correlation_matrix = metrics.get_correlation_matrix(
        params=["param1", "param2"]
    )
    assert correlation_matrix.shape == (2, 2)
    assert (
        -1 <= correlation_matrix[0, 1] <= 1
