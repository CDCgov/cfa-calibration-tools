...
def test_resolve_cloud_sizing_from_config_uses_local_task_probe(
    monkeypatch,
    tmp_path,
):
    config_path = _write_cloud_config(
    ...
    assert sizing.task_slots_per_node_override == 5
    assert captured["probe_module"] == "calibrationtools.cloud.auto_size"
    assert captured["base_inputs"] == {
        "probe_kind": "local_task",
        "cloud_config_path": str(config_path),
        "mrp_config_path": str(tmp_path / "model.mrp.toml"),
        "input": {"seed": 3},
    }
    assert captured["memory_scope"].value == "process_tree"
