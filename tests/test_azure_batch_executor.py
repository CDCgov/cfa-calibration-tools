...
def test_azure_executor_keeps_download_chatter_out_of_the_console(
    ...
    """Keep routine per-blob download output out of a caller's display.

    Harvesting runs inside the poll loop, so anything the library prints or
    logs there lands in the middle of a live display. A blob that is not
    readable yet is expected while the job runs, and the library reports it at
    ERROR, so neither the chatter nor the miss belongs on the progress feed.
    """
    ...
    assert not any("Size of file to download" in notice for notice in notices)
