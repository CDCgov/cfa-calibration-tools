...
def test_cleanup_listing_one_session_matches_truncated_resource_names():
    truncated_session_tail = SESSION_ID[-20:]
    listing = discover(
        make_client(
            pool_names=[f"pool-{truncated_session_tail}"],
            job_names=[f"job-{truncated_session_tail}-j1"],
            container_names=[f"input-{truncated_session_tail}"],
        ),
        session_id=SESSION_ID,
    )

    assert listing.pool_names == (f"pool-{truncated_session_tail}",)
    assert listing.job_names == (f"job-{truncated_session_tail}-j1",)
    assert listing.container_names == (f"input-{truncated_session_tail}",)
...
def test_user_cleanup_plans_include_truncated_resources_for_matching_sessions():
    truncated_session_tail = SESSION_ID[-20:]
    plans = discover_cleanup_plans_for_user(
        make_client(
            pool_names=[f"pool-{SESSION_ID}"],
            job_names=[f"job-{truncated_session_tail}-j1"],
            container_names=[f"input-{truncated_session_tail}"],
        ),
        make_settings(),
    ...
    assert len(plans) == 1
    assert plans[0].session_id == SESSION_ID
    assert plans[0].pool_names == (f"pool-{SESSION_ID}",)
    assert plans[0].job_names == (f"job-{truncated_session_tail}-j1",)
    assert plans[0].container_names == (f"input-{truncated_session_tail}",)


def test_format_cleanup_plans_prints_shared_header_once():
    plans = (
        CleanupPlan(
            config_path=Path("cloud.toml"),
            session_id=SESSION_ID,
            ...
            job_names=("job-1",),
            pool_names=("pool-1",),
            container_names=("input-1",),
            acr_image_exists=False,
        ),
        CleanupPlan(
            config_path=Path("cloud.toml"),
            session_id=ALICE_SECOND_SESSION_ID,
            keyvault="kv",
            registry_name=None,
            repository_name="repo",
            image_tag=None,
            job_names=("job-2",),
            pool_names=("pool-2",),
            container_names=("input-2",),
            acr_image_exists=False,
        ),
    )

    formatted = format_cleanup_plans(plans, include_acr=True)

    assert formatted.count("Cloud cleanup plan") == 1
    assert formatted.count("  config: cloud.toml") == 1
    assert formatted.count("  keyvault: kv") == 1
    assert f"  session: {SESSION_ID}" in formatted
    assert f"  session: {ALICE_SECOND_SESSION_ID}" in formatted
