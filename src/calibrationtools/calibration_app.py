...
from .cloud.runner import create_cloud_mrp_runner_from_config
from .mrp_runner import MRPOutputRunner
from .output_contracts import CSVColumnOutputContract, OutputContract
...
def _create_mrp_runner(
    config_path: Path,
    spec: CalibrationAppSpec,
) -> MRPOutputRunner[Any]:
    """Create an MRP runner from the model output contract."""
...
def resolve_model_runner(
    ...
    *,
    ...
    if args.mrp_config is not None:
        return _create_mrp_runner(args.mrp_config, spec)
    if args.docker:
        if spec.default_docker_mrp_config_path is None:
            raise ValueError("--docker requires a default Docker MRP config")
        return _create_mrp_runner(spec.default_docker_mrp_config_path, spec)
