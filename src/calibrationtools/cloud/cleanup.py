...
def main(argv: list[str] | None = None) -> int:
    ...
    return run_cleanup(argv, default_config_path=Path("cloud_config.toml"))
...
def run_cleanup(
    ...
    *,
    ...
    """Run cleanup with a caller-supplied default cloud config."""
    args = parse_args(
        argv,
        default_config_path=default_config_path,
        description=description,
...
def _discover_cleanup_batch_resources(
    ...
    *,
    ...
    if user is not None:
        ...
        matching_session_ids = _session_ids_for_user(
            job_names,
            pool_names,
            container_names,
            normalized_user,
        ...
        if (
            ...
            and parse_username_from_session_id(session_id) == normalized_user
        ):
            matching_session_ids = tuple(
                sorted({*matching_session_ids, session_id})
            )
...
def _format_cleanup_plan_session(
    plan: CleanupPlan,
    *,
    include_acr: bool,
    include_keyvault: bool,
) -> list[str]:
    lines = [f"  session: {plan.session_id}"]
    if include_keyvault:
        lines.append(f"  keyvault: {plan.keyvault}")
    lines.append(f"  batch jobs: {len(plan.job_names)}")
...
def format_cleanup_plans(
    ...
    *,
    ...
    lines = [
        "Cloud cleanup plan",
        f"  config: {plans[0].config_path}",
        f"  keyvault: {plans[0].keyvault}",
        ...
        sorted(
            {
                ...
                and (
                    session_id in session_id_getter(name)
                    or _resource_name_suffix_matches_session_id(
                        name,
                        session_id,
                        allowed_prefixes,
                    )
...
def _filter_names_for_session_user(
    ...
    session_id_getter: Callable[[str], tuple[str, ...]],
    allowed_prefixes: tuple[str, ...],
    matching_session_ids: tuple[str, ...],
) -> tuple[str, ...]:
    return tuple(
        sorted(
            {
                ...
                or any(
                    _resource_name_suffix_matches_session_id(
                        name,
                        session_id,
                        allowed_prefixes,
                    )
...
def _session_ids_for_user(
    ...
    return tuple(
        sorted(
            ...
            if parse_username_from_session_id(session_id) == user
        )
    )
...
def _session_ids_for_job_name(job_name: str) -> tuple[str, ...]:
    base_name = re.sub(r"-j\d+$", "", job_name)
    return _session_ids_for_resource_name(base_name)


def _session_ids_for_resource_name(resource_name: str) -> tuple[str, ...]:
    match = re.search(
        r"(?P<session>\d{14}-[a-z0-9]+(?:-[a-z0-9]+)*-[a-z0-9]+-[0-9a-f]{12})$",
        resource_name,
    )
...
def _resource_name_suffix_matches_session_id(
    name: str,
    session_id: str,
    allowed_prefixes: tuple[str, ...],
) -> bool:
    normalized_session_id = sanitize_name(session_id)
    for prefix in allowed_prefixes:
        suffix = _resource_suffix_after_prefix(name, prefix)
        if suffix is None:
            continue
        suffix = re.sub(r"-j\d+$", "", suffix)
        if suffix and normalized_session_id.endswith(suffix):
            return True
    return False


def _resource_suffix_after_prefix(name: str, prefix: str) -> str | None:
    ...
    if name.startswith(prefix_with_separator):
        return name[len(prefix_with_separator) :]
    return None
