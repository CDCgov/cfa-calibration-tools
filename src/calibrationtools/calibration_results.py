...
class CalibrationResults:
    ...
    Stores the results of the calibration process, including the posterior particle population, population archive, success counts, and other relevant information.
     ...
     Args:
        posterior_particle_population (ParticlePopulation): The final particle population representing the posterior distribution after calibration.
        ...
        originator_perturbation_kernel (PerturbationKernel): The original perturbation kernel used for sampling particles in the initial generation.
        posterior_perturbation_kernel (PerturbationKernel): The perturbation kernel used for sampling particles in the final generation, which may have been adapted during the calibration process.
    ...
    def __init__(
            ...
            originator_perturbation_kernel: PerturbationKernel,
        ...
        self.generation_particle_count = success_counts["generation_particle_count"]
        ...
        self.originator_perturbation_kernel = originator_perturbation_kernel
        self.priors = priors

        self.aggregate_acceptance_rate = sum(self.smc_step_successes) / sum(self.smc_step_attempts) if sum(self.smc_step_attempts) > 0 else 0
    ...
    def acceptance_rates(self) -> list[float]:
        return [
            successes / attempts if attempts > 0 else 0 for successes, attempts in zip(self.smc_step_successes, self.smc_step_attempts)
        ]
    ...
    def _validate(self):
        ...
        Raises:
            ValueError: If posterior particle population is not normalized
            ValueError: If the length of success counts does not match the length of tolerance values
            ValueError: If the final step successes do not match the generation particle count
            ValueError: If the final step successes do not match the particle population size
        """

        if (self.posterior.particle_population.total_weight - 1.0) > 1e-6:
            raise ValueError(f"Particle population weights should already be normalized to 1.0, instead got {self.posterior.particle_population.total_weight}")

        if len(self.smc_step_successes) != len(self.tolerance_values):
            raise ValueError("Length of success counts should match length of tolerance values")
        if self.smc_step_successes[-1] != self.generation_particle_count[-1]:
            raise ValueError("Step successes should match generation particle count for the final generation")
        if self.smc_step_successes[-1] != self.posterior.particle_population.size:
            raise ValueError("Step successes should match particle population size for the final generation")
    
    def sample_posterior_particles(self, n: int=1, perturb: bool = False) -> list[Particle]:
        """
        ...
        """
        if perturb:
            return [self._updater.sample_and_perturb_particle() for _ in range(n)]
        else:
    ...
    def get_diagnostics(self) -> dict[str, Any]:
        ...
        """
        ess_vals = [p.ess for p in self.population_archive.values() if not p.is_empty()]
        ...
        return {
            ...
            "quantiles": self.posterior.get_quantiles(params=self.fitted_params),
            "posterior_weights": self.posterior.particle_population.weights,
            "covariance_matrix": self.posterior.get_covariance_matrix(self.fitted_params),
            "correlation_matrix": self.posterior.get_correlation_matrix(self.fitted_params)
        }