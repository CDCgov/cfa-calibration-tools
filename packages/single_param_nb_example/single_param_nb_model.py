...
def NB_n_Model(model_inputs: dict[str, Any]) -> list[int]:
        ...
        for i in range(model_inputs["max_gen"]):
            if i == 0:
                ...
                next = int(sum(rng.binomial(model_inputs["n"], model_inputs["p"], out[i - 1])))
                out.append(next)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run NB_n_Model with parameters from JSON file")
    parser.add_argument("json_file", help="Path to JSON file containing model parameters")
    parser.add_argument("-o", "--output", default="model_output.json", 
