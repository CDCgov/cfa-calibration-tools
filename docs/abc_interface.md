...
class ABCSMC:
    ...
    def run():
        for g in range(G):
            while i < N:
                if g == 1:
                    particle = sample_particle_from_priors()
                else:
                    particle = sample_from_previous_generation(weights)
                    perturbed_particle = perturb_particle(particle)
                    er = calculate_distance(particle)
                if er < d[g]:
                    calculate_weight()
